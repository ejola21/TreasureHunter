...
- (void)setupView {
	...
	currentIdx = -1;
	star = [[UIImage imageNamed:@"star-white32.png"] retain];    
	highlightedStar = [[UIImage imageNamed:@"star-gold32.png"] retain];    

	for (int i=0; i<numberOfStars; i++) {
		DLStarView *v = [[DLStarView alloc] initWithDefault:self.star highlighted:self.highlightedStar position:i allowFractions:isFractionalRatingEnabled starSize:size];
		[self addSubview:v];
		[v release];
	}
}
...
- (id)initWithFrame:(CGRect)frame 
       ...
       setSize0to20:(int)getSize 
       clickEnabled:(BOOL)clickEnabled{
    isClickEnable = clickEnabled;
    size = 20-getSize;
	self = [super initWithFrame:frame];
	if (self) {
